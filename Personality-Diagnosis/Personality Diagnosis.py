...
testData={}  # 25% of total movies for each user
trainData={} # 75% of total movies for each user
...
for user in dataset:
    current=0
    totalMovies=len(dataset[user])
    totalMovies=int(totalMovies/4)
    tempTest={}
    tempTrain={}
    for movie in dataset[user]:
        if(current<totalMovies):
            tempTest[movie]=dataset[user][movie]
        else:
            tempTrain[movie]=dataset[user][movie]
        current+=1
    testData[user]=tempTest
    trainData[user]=tempTrain
...
def Analysis():
    ...
    for user in testData:
        ...
        for movie in testData[user]:
            ...
            sq_sum+=abs(actual_rating-predicted_rating)
            print(abs(actual_rating-predicted_rating))
            n=n+1
    sq_sum/=n
    ##sq_sum=sqrt(sq_sum)
    print sq_sum
